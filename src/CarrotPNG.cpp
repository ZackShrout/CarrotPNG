...
namespace cpng {
    [[nodiscard]] decode_error read_ihdr_from_memory(const std::span<const uint8_t> data, ihdr_info_t& out_ihdr) noexcept
    {
        out_ihdr = { }; // clear
        ...
        std::span<const uint8_t> ihdr_data{ data.begin() + pos, 13 };
        pos += 13;
        ...
        const uint32_t actual_crc = crc32_finalize(crc);
        ...
        // Parse IHDR fields
        size_t ihdr_pos = 0;
        auto w = read_be_uint32_t(ihdr_data, ihdr_pos);
        auto h = read_be_uint32_t(ihdr_data, ihdr_pos);
        auto bd = read_uint8_t(ihdr_data, ihdr_pos);
        auto ct = read_uint8_t(ihdr_data, ihdr_pos);
        auto cm = read_uint8_t(ihdr_data, ihdr_pos);
        auto fm = read_uint8_t(ihdr_data, ihdr_pos);
        auto im = read_uint8_t(ihdr_data, ihdr_pos);
    ...
    [[nodiscard]] decode_error load_from_memory(const std::span<const uint8_t> data, image_view_t& out_view, const std::span<uint8_t> out_rgba8) noexcept
    {
